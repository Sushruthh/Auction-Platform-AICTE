...
export const addNewAuctionItem = catchAsyncErrors(async(req,res,next) => {
    ...
    if(new Date(startTime) < Date.now()){
        return next(new ErrorHandler("You cannot start an auction before current time!", 400));
    }
    ...
    try {
        ...
        return res.status(201).json({
            success: true,
            message: `Auction item created and will be listed on auctions page at ${startTime}`,
            auctionItem,
        })
    } catch (error) {
